...
class PSPNet(nn.Cell):
    def __init__(self, backbone="resnet50", pool_sizes=(1, 2, 3, 6), num_classes=2, pretrained=False,
    ...
    def construct(self, x):
        ...
        if self.training:
            aux = self.aux(x_tmp)
            aux = self.resize(aux, size=(h, w), align_corners=True)
            # aux = ops.interpolate(aux, sizes=(h, w), mode='bilinear', coordinate_transformation_mode="align_corners")
            return aux, x
        else:
            return x
